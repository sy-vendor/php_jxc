...
class DedeSql
{
	...
	function Open($pconnect=false)
	{
		...
		{
		  if(!$pconnect){ $this->linkID  = @mysqli_connect($this->dbHost,$this->dbUser,$this->dbPwd, $this->dbName); }
		  else{ 
		...
		if(!$this->linkID){
			//echo $this->GetError();
			$this->DisplayError("OACIO错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错，如未安装本系统，请先运行安装程序，如果已经安装，请检查MySQL服务或修改include/config_base.php的配置！</font>");
			exit();
		}
		@mysqli_select_db($this->dbName);
		$mysqlver = explode('.',$this->GetVersion());
		$mysqlver = $mysqlver[0].'.'.$mysqlver[1];
		if($mysqlver>4.0) @mysqli_query($this->linkID, "SET NAMES '".$GLOBALS['cfg_db_language']."';");
		if($mysqlver>5.0) @mysqli_query($this->linkID, "SET sql_mode='' ;");
		return true;
	}
	//
	//获得错误描述
	...
	function ClearErrLink()
	{
		...
		@$result=mysqli_query("SHOW PROCESSLIST",$this->linkID);
    if($result)
    {
       while($proc=mysqli_fetch_assoc($result))
       {
          if($proc['Command']=='Sleep'
             && $proc['Time']>$cfg_dbkill_time) @mysqli_query($this->linkID, "KILL ".$proc["Id"]);
       }
    }
	}
	...
	function Execute($id="me",$sql="")
	{
		...
		if(!$this->result[$id]){
			$this->DisplayError(mysqli_error()." - Execute Query False! <font color='red'>".$this->queryString."</font>");
		}
	}
	...
	function GetOne($sql="",$acctype=MYSQL_BOTH)
	{
		...
		if($sql!=""){
		  if(!preg_replace("/limit/i",$sql)) $this->SetQuery(preg_replace("/[,;]$/i","",trim($sql))." limit 0,1;");
	...
	function GetArray($id="me",$acctype=MYSQL_BOTH)
	{
		...
		else return mysqli_fetch_array($this->result[$id],$acctype);
	}
	function GetObject($id="me")
	{
		if($this->result[$id]==0) return false;
		else return mysqli_fetch_object($this->result[$id]);
	}
	//
	//检测是否存在某数据表
	//
	...
	function GetTotalRow($id="me")
	{
		...
		else return mysqli_num_rows($this->result[$id]);
	}
	//
	//获取上一步INSERT操作产生的ID
	//
	...
	function DisplayError($msg)
	{
		...
		echo "<title>WEB ERP SYSTEM Error Track</title>\r\n";
		echo "</head>\r\n";
